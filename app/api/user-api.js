...
export function getUsers() {
  return axios.get('../../data/users.json')
    .then(response => {
      //response=[{"id":1,"name":"Dev"},{"id":2,"name":"Demmy"}];
...
export function searchUsers(query = '') {
  return axios.get('../../data/users.json?q='+ query)
    .then(response => {
      store.dispatch(getUsersSuccess(response.data));
      return response;
    });
...
export function deleteUser(userId) {
  return axios.delete('../../data/users.json' + userId)
    .then(response => {
...
export function getProfile(userId) {
  ...
  return axios.get('../../data/users.json' + userId)
    .then(response => {
